...
export class TechChoiceRule {
     ...
     * 提取被拒绝的选项
     */
    extractRejected(content) {
        const rejected = [];
        // 匹配 "在 A 和 B 之间选择 C" 格式
        // 注意：如果选择的是 A 或 B 其中之一，另一个就是被拒绝的
        const betweenMatch = content.match(/在\s+([A-Z][a-zA-Z0-9.]+)\s+和\s+([A-Z][a-zA-Z0-9.]+)\s+(?:之间)?(?:选择|决定使用|采用)\s+([A-Z][a-zA-Z0-9.]+)/);
        if (betweenMatch) {
    ...
    extractTags(content) {
        const tags = [];
        // 检测技术栈关键词
        if (/前端|web|ui|ux|css|html|react|vue|angular/i.test(content))
            tags.push('前端');
        if (/后端|backend|api|服务端|server/i.test(content))
            tags.push('后端');
        if (/数据库|存储|db|database|sql|nosql/i.test(content))
            tags.push('数据库');
        if (/部署|运维|devops|ci|cd|docker/i.test(content))
            tags.push('运维');
        if (/移动端|ios|android|app|移动/i.test(content))
            tags.push('移动端');
        if (/测试|test|单元测试|集成测试/i.test(content))
            tags.push('测试');
        return tags;
    }
    /**
     * 计算置信度
     */
    calculateConfidence(content, reason, dimensions) {
        ...
        if (dimensions.length > 0)
            confidence += 0.1 * Math.min(dimensions.length, 3);
        // 内容长度适中（太短或太长都不可靠）
...
//# sourceMappingURL=tech-choice.js.map